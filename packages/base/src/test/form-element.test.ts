...
@customElement('custom-focus-form-element')
class CustomFocusFormElement extends FormElement {
  @query('#root') protected mdcRoot!: HTMLElement;
...
const testFocusFormElement = html`
  <custom-focus-form-element></custom-focus-form-element>
`;
...
suite('form-element:', () => {
  ...
  suite('test-focus-form-element', () => {
    let fixt: TestFixture;
    let component: CustomFocusFormElement|null;
    let formElement: HTMLElement|null;
    let indirectFormElement: HTMLElement|null;

    setup(async () => {
      fixt = await fixture(testFocusFormElement);
      component = fixt.root.querySelector('custom-focus-form-element');
    ...
    test('is an instance of form-element', () => {
      assert.instanceOf(component, FormElement);
      assert.instanceOf(component, CustomFocusFormElement);
    });

    test('shadowRoot does not focus on formElement after click', async () => {
      if (component === null) {
        assert.isNotNull(component);
        return;
      }
    ...
    test('shadowRoot focuses on indirectFormElement after click', async () => {
      if (component === null) {
        assert.isNotNull(component);
        return;
      }
      ...
      assert.equal(component.shadowRoot.activeElement, indirectFormElement);
    });
  });
